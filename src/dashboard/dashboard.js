...
// ══════════════════════════════════════════════════════
//  DRAG & DROP / CLIPBOARD PASTE — Image Upload System
// ══════════════════════════════════════════════════════
...
/**
 * Trova la recipe-card o recipe-row più vicina al target dell'evento.
 */
...
/**
 * Inizializza il sistema completo di Drag & Drop + Clipboard Paste.
 * Usa event delegation su #recipesGrid per funzionare anche dopo re-render.
 */
function initDragAndDrop() {
    const grid = document.getElementById('recipesGrid');
    if (!grid) return;

    // ── Prevent default su tutto il body (evita che il browser apra l'immagine) ──
    document.addEventListener('dragover', e => e.preventDefault());
    document.addEventListener('drop', e => e.preventDefault());

    // ── DRAGOVER: evidenzia la card ──
    grid.addEventListener('dragover', (e) => {
        e.preventDefault();
        e.dataTransfer.dropEffect = 'copy';
        const card = findCardFromEvent(e);
        if (card && !card.classList.contains('drag-over')) {
            // Rimuovi da tutte le altre
            grid.querySelectorAll('.drag-over').forEach(c => c.classList.remove('drag-over'));
            card.classList.add('drag-over');
        }
    });

    // ── DRAGLEAVE: rimuovi evidenziazione ──
    ...
    grid.addEventListener('drop', async (e) => {
        ...
        // Priorità 2: URL immagine (drag da browser)
    ...
    // ── CLIPBOARD PASTE: Ctrl+V su card con focus ──
    document.addEventListener('paste', (e) => {
        // Solo se siamo nel pannello ricette
        ...
        // Cerca un'immagine negli item della clipboard
        let imageItem = null;
        ...
        // Trova la card selezionata (o l'unica con focus)
    ...
    console.log('🖱️ Drag & Drop + Clipboard Paste inizializzato');
}

/**
 * Gestisce un file immagine locale (da drop o paste).
 * Mostra preview overlay con conferma/annulla.
 */
function handleImageFile(file, slug, category, card) {
    ...
    // Crea blob URL per preview immediata
    const blobUrl = URL.createObjectURL(file);
    showPreviewOverlay(card, blobUrl, slug, category, () => {
        // Callback conferma: leggi come Base64 e invia
...
/**
 * Gestisce un URL immagine (drag da browser).
 * Mostra preview overlay con l'URL.
 */
...
/**
 * Mostra un overlay di preview sull'immagine della card.
 * Include pulsanti Conferma / Annulla.
 */
...
/**
 * Invia immagine Base64 al backend.
 * Optimistic UI: mostra l'immagine immediatamente sulla card.
 */
async function uploadImageBase64(slug, category, dataUrl, card) {
    // Optimistic UI: mostra subito l'immagine
...
/**
 * Invia URL immagine al backend (il server la scaricherà).
 */
...
/**
 * Mostra/nasconde stato di caricamento sulla card.
 */
